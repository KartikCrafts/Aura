...
export interface InfiniteSpiralItemObject {
  id?: string | number;
  src: string;
  alt?: string;
  label?: string;
...
export type InfiniteSpiralItem = string | InfiniteSpiralItemObject;
...
export interface InfiniteSpiralProps {
  ...
  grayscale?: number;
  className?: string;
  onCardClick?: (item: InfiniteSpiralItemObject, index: number) => void;
}
...
export const InfiniteSpiral: React.FC<InfiniteSpiralProps> = ({
  ...
  const normalizedItems = useMemo<InfiniteSpiralItemObject[]>(
    () =>
      items.map((item, index) =>
        typeof item === 'string'
          ? { src: item, alt: `Spiral estate ${index + 1}`, label: `Estate ${index + 1}` }
          : { alt: `Spiral estate ${index + 1}`, ...item }
      ),
    [items]
  );
  ...
  useEffect(() => {
    ...
    // Note: Web page scroll listener removed intentionally as requested:
    // Web scroll up/down will NOT affect or rotate the spiral. Only smooth auto rotation and drag.
        ...
        {normalizedItems.map((item, index) => {
          ...
          return (
            <Card
              ...
              onClick={() => {
                if (!dragMovedRef.current && onCardClick) {
            ...
            >
              ...
              {(item.label || item.price) && (
                <div className="infinite-spiral__badge">
                  {item.label && <span className="infinite-spiral__title">{item.label}</span>}
                  {item.price && <span className="infinite-spiral__price">{item.price}</span>}
                </div>
              )}
            </Card>
          );
