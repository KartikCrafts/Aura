  ...
  const pushSiblings = (hoveredIdx: number) => {
    ...
    images.forEach((_, i) => {
      ...
      if (i === hoveredIdx) {
        ...
        gsap.to(selector, {
          transform: `${noRotation} scale(1.12)`,
          zIndex: 30,
          duration: 0.4,
          ease: 'back.out(1.4)',
          overwrite: 'auto'
        ...
        gsap.to(selector, {
          transform: `${pushedTransform} scale(0.92)`,
          zIndex: i,
          duration: 0.4,
          ease: 'back.out(1.4)',
          delay,
  ...
  return (
    ...
    >
      {images.map((src, idx) => (
        <div
          key={idx}
          className={`card card-${idx} absolute w-[200px] aspect-square border-4 border-[#d9ccb6] rounded-[24px] overflow-hidden cursor-pointer`}
          style={{
